...
import type { SocialProofScenario } from "@/lib/constants/social-proof";

type SocialProofCardProps = {
  scenario: SocialProofScenario;
  onAccept: () => void;
  onDismiss: () => void;
};
      ...
      >
        ...
        <div className="mb-3 flex items-center justify-between rounded-xl border border-qnb-purple/25 bg-gradient-to-r from-qnb-purple/10 to-qnb-navy/5 px-3 py-2.5">
          <div>
            ...
            <p className="text-2xl font-semibold tabular-nums text-qnb-purple">
              %{scenario.adoptionPercent}
            </p>
          </div>
          <div className="text-right">
            <p className="text-[9px] text-body">Trend hedef</p>
            <p className="max-w-[120px] truncate text-[11px] font-medium text-label">
              {scenario.trendingGoal}
            </p>
          </div>
        </div>

        <div className="mb-3 h-1.5 overflow-hidden rounded-full bg-qnb-navy/10">
          <div
            className="h-full rounded-full bg-gradient-to-r from-qnb-navy to-qnb-purple"
            style={{ width: `${scenario.adoptionPercent}%` }}
          />
        </div>
        ...
        <div className="mt-3 rounded-lg border border-qnb-navy/10 bg-qnb-navy/5 px-3 py-2.5">
          <p className="text-[9px] font-medium uppercase tracking-wide text-qnb-navy">
            Önerilen küçük adım
          </p>
          <p className="mt-0.5 text-sm font-semibold tabular-nums text-heading">
            {scenario.suggestedAmount.toLocaleString("tr-TR")} TL/ay ·{" "}
            {scenario.trendingGoal}
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={onDismiss}
          className="mt-2 w-full rounded-full border border-border-default py-2.5 text-[12px] font-medium text-body transition hover:bg-surface-alt focus:outline-none focus-visible:ring-2 focus-visible:ring-qnb-navy"
        >
